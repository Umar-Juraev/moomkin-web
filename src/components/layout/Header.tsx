...
import { Globe, HeartIcon, MapPin, Settings, AlertCircle, LogOut, X } from "lucide-react";
    ...
    <div className="flex justify-between align-center bg-white px-10 h-20 md:h-auto md:block md:px-4">
      ...
      >
        ...
        <DropdownMenu onOpenChange={(isOpen) => setBurger(isOpen)}>
          <DropdownMenuTrigger asChild className="md:flex  w-12 h-12 rounded-full bg-main-light-gray hover:opacity-60 hidden justify-center items-center p-0 m-0 border-none shadow-none">
            {!burger ?
              <Button variant="outline" className="p-0">
                <Image src={burgerIcon} alt="menu" />
              </Button>
              : <div><X size={25} color="#292C30" /></div>}
          </DropdownMenuTrigger>
          <DropdownMenuContent
          ...
          >
            <DropdownMenuGroup className="p-4 m-0">
              {false ?
                ...
                : <Button className="w-full text-base font-bold">{t("login")}</Button>
              }
            </DropdownMenuGroup>
            <DropdownMenuSeparator className="bg-[#919DA63D]" />
            <DropdownMenuGroup className="p-1">
              <DropdownMenuItem onClick={() => router.push('/favorites')} className="cursor-pointer text-base font-medium p-3 relative">
                {favorites.length ?
                  <Badge
                  ...
                  >
                    {favorites.length}
                  </Badge> : <HeartIcon size={34} color="#292C30" />}
                Saved</DropdownMenuItem>
              <DropdownMenuItem className="cursor-pointer text-base font-medium p-3"> <Settings size={34} color="#292C30" /> Settings</DropdownMenuItem>
              <DropdownMenuItem className="cursor-pointer text-base font-medium p-3"> <AlertCircle size={34} color="#292C30" className="rotate-180" /> About the program</DropdownMenuItem>
            </DropdownMenuGroup>
            <DropdownMenuSeparator className="bg-[#919DA63D]" />
            <DropdownMenuGroup className="p-1">
              {true &&
                <DropdownMenuItem className="cursor-pointer text-base font-medium p-3">
                  <LogOut size={34} className="rotate-180" color="#292C30" /> {t("logout")}
                </DropdownMenuItem>
              }
            </DropdownMenuGroup>
          </DropdownMenuContent>
